...
export function useGachaBoxInfoList() {
  const { loading: loadingGachaBoxes, collection: gachaBoxes }
    = useCollection('GachaBoxes');
  const { loading: loadingCharacterCards, collection: characterCards }
    = useCollection('CharacterCards');
  ...
  const loading = computed(
    () =>
      loadingGachaBoxes.value
      || loadingCharacterCards.value
      || loadingTerms.value,
  );
  const gachaBoxInfoList: Ref<GachaBoxInfo[]> = computed(() =>
    loading.value
      ? []
